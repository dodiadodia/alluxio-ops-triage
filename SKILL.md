...
- Treat every Alluxio properties change as expert-gated, including direct edits to `*.properties` files and equivalent changes through a ConfigMap, AlluxioCluster, Helm values, environment variables, JVM `-D` options, or component startup arguments. Reading, comparing, and drafting a proposed change are allowed, but never apply the change until the exact keys and values have been manually reviewed and explicitly approved by an Alluxio expert. Do not infer expert approval from a general request to fix, tune, or modify configuration, from approval of another action, or from silence.
- Immediately before requesting properties approval, show the exact current effective value and source when available, proposed value, affected components and scope, version-specific evidence, expected restart or rollout, customer impact, validation, and rollback. Then stop and ask the user to confirm that an Alluxio expert has approved those exact keys and values. If that confirmation is absent or ambiguous, provide only the pending change plan and escalate to an Alluxio expert.
